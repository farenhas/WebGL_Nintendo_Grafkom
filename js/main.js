...
async function main() {
    ...
    const parts = obj.geometries.map(({ material, data }) => {
        ...
        // Set up color data if available, otherwise use default
        if (data.color) {
          if (data.position.length === data.color.length) {
            // if colors are given as an array matching positions
            data.color = { numComponents: 3, data: data.color };
          }
        } else {
          // if no vertex colors, use diffuse color from material or default to white
